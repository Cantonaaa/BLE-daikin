...
static volatile bool rename_pending = false;

static voice_power_fn power_cb = NULL;
static voice_get_units_fn get_units_cb = NULL;
...
void voice_control_sync_commands(void)
{
    ...
    for (int i = 0; i < cnt; i++) {
        ...
        esp_mn_commands_add(ids[i], cmd_on);
        esp_mn_commands_add(ids[i] | 0x100, cmd_off);
    }
